...
//        cv::namedWindow("Image", cv::WINDOW_NORMAL);
    for(int i=0; i<samplesNum; i++){
        ...
        cv::Mat Image = cv::imread(mImageLabels.at(i).imagePath, cv::IMREAD_GRAYSCALE); //CV_LOAD_IMAGE_GRAYSCALE
...
//            cv::waitKey(10);
    ...
    for(int i=0; i<LinearRegressors.size(); i++){
        ...
        for(int j=0; j<samplesNum; j++){
            cv::Mat grayImage = cv::imread(mImageLabels.at(j).imagePath, cv::IMREAD_GRAYSCALE);
